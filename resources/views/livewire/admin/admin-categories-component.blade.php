...
<div class="card-body">
    ...
    <table class="table table-striped">
        ...
        @foreach($categories as $category)
            <tr>
                <td>{{++$i}}</td>
                <td><img src="{{asset('assets/imgs/category')}}/{{$category->image}}" width="80"></td>
                <td>{{$category->name}}</td>
                <td>{{$category->slug}}</td>
                <td>{{$category->is_popular}}</td>
                <td>
                    <a href="{{route('admin.categories.edit',['category_id'=>$category->id])}}" class="btn btn-secondary">Edit</a>
